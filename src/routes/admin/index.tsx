    ...
    <div className="min-h-screen bg-secondary/35">
      ...
      <main className="mx-auto max-w-[1500px] px-5 py-8 sm:px-8">
        ...
        <Tabs defaultValue="orders">
          <TabsList className="mb-5"><TabsTrigger value="orders">Orders</TabsTrigger><TabsTrigger value="products">Products</TabsTrigger></TabsList>
          <TabsContent value="orders"><Card><CardHeader className="gap-4 border-b border-border sm:flex-row sm:items-center sm:justify-between"><div><CardTitle>Order queue</CardTitle><p className="mt-1 text-sm text-muted-foreground">Payment, delivery, and customer coordination.</p></div><div className="relative w-full sm:w-72"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search orders" className="pl-9" /></div></CardHeader><CardContent className="p-0"><OrdersTable orders={visibleOrders} onSelect={async (id) => { try { setSelectedOrder(await useServerFn(getAdminOrder)({ data: { id } })); } catch { toast.error("Could not open that order."); } }} /></CardContent></Card></TabsContent>
